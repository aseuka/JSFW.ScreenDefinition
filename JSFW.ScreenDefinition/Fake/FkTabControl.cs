...
namespace JSFW.Fake
{
    public partial class FkTabControl : Panel, IFkData
    {
        Popup Editor = null;
        EditorFkTabControl TabEditor = null;

        public FkData Data { get; internal set; } = new FkData()
        {
            TypeName = typeof(FkTabControl).FullName,
            Text = "탭",
            Visible = true,
            Left = 0,
            Top = 0,
        ...
        public void SetData(FkData data)
        {
            try
            {
                isDataBinding = true;
                Data = data;
                Padding = new Padding(0, 3 + 22 + 3, 0, 3);
                ...
                if (Data != null)
                {
                    ...
                    {
                        if (childData.TypeName == typeof(FkTabPage).FullName)
                        {
                            SizeF ssz = TextRenderer.MeasureText(childData.Text, Font);
                            childData.HeadBox = new Rectangle(x, this.Top, (int)ssz.Width + 6, 22);
                            x += (int)ssz.Width + 6 + 2;
                        }
                    ...
                    if (data.Childs.Any(d => d.Selected) == false && 0 < data.Childs.Count)
                    {
                        data.Childs[0].Selected = true;
                    }
                }
                Invalidate(true);
            }
        ...
        public FkTabControl()
        {
            ...
            Data.Childs.Add(new FkTabPage().Data);
            Data.Childs[0].Text = "탭 페이지1";
            Data.Childs[0].Selected = true;
            SetData(Data);

            Editor = new Popup(TabEditor = new EditorFkTabControl());
        ...
        {
            if ((e.Control is FkTabPage) == false)
            {
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            FkTabPage selectedFkTabPage = null;
            foreach (FkTabPage page in Controls)
            {
                FkData fkD = page.Data;
                {
                    if (fkD.HeadBox.HasValue &&
                        fkD.HeadBox.Value.Contains(e.Location))
                    {
                        selectedFkTabPage = page;
                    }
                }
            }

            if (selectedFkTabPage != null)
            {
                selectedFkTabPage.Data.Selected = true;
                foreach (FkTabPage page in Controls)
                {
                    page.Visible = page.Equals(selectedFkTabPage);
                    page.Data.Selected = page.Visible;
                }
                Invalidate();
            }
        }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            Rectangle textRect = Rectangle.FromLTRB(
                ...
                ClientRectangle.Right - Margin.Right,
                ClientRectangle.Top + 3 + 22);

            Point pt = new Point(DisplayRectangle.Left + Margin.Left + 1,
                                 DisplayRectangle.Top + Margin.Top + 1);
            ...
            if (!string.IsNullOrWhiteSpace(txt))
            {
                TextRenderer.DrawText(e.Graphics, txt, Font, textRect,
                    foreClr, TextFormatFlags.VerticalCenter | TextFormatFlags.Right);

                SizeF sz = TextRenderer.MeasureText(txt, Font);
                e.Graphics.DrawLine(Pens.DarkGray, textRect.Left, textRect.Top + 22, textRect.Right, textRect.Top + 22);
            }
            ...
            if (Data != null)
            {
                try
                {
                    FkData selectedFkData = null;
                    foreach (FkTabPage page in Controls)
                    {
                        FkData fkD = page.Data;
                        if (selectedFkData == null || fkD.Selected)
                        {
                            selectedFkData = fkD;
                        }
                    }

                    if (selectedFkData != null && !selectedFkData.Selected)
                    {
                        selectedFkData.Selected = true;
                    }
                    ...
                    foreach (FkTabPage page in Controls)
                    {
                        FkData fkD = page.Data;
                        {
                            SizeF ssz = TextRenderer.MeasureText(fkD.Text, Font);
                            fkD.HeadBox = new Rectangle(x, 3, (int)ssz.Width + 6, 22);
                            x += (int)ssz.Width + 6 + 2;

                            if (fkD.Selected)
                            {
                                e.Graphics.FillRectangle(Brushes.LightGray, fkD.HeadBox.Value);
                            }
                            TextRenderer.DrawText(e.Graphics, $"[{fkD.Text}]", Font, fkD.HeadBox.Value,
    ...
    [Browsable(false)]
    [ToolboxItem(false)]
    internal partial class FkTabPage : Panel, IFkData
    {
        public FkData Data { get; internal set; } = new FkData()
        {
        ...
        /// 80
        /// </summary>
        readonly int _Width = 51; // 라벨100 = 기본
        /// <summary>
        /// 28
        /// </summary>
