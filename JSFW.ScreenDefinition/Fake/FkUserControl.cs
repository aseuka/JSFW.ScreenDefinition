...
namespace JSFW.Fake
{
    public partial class FkUserControl : UserControl, IFkData
    {
        ...
        protected override void OnDragDrop(DragEventArgs e)
        {
            ...
            if (e.AllowedEffect == DragDropEffects.Copy && e.Data.GetDataPresent(typeof(TooBoxItemDragDropData).FullName))
            {
                ...
                if (dropObject != null )
                {
                    if (!string.IsNullOrWhiteSpace(dropObject.TypeName))
                    {
    ...
    public class DragMoveControlObject : IDisposable
    {
        public Control MoveControl { get; set; }

        //드래그 중에 컨트롤들을 옮겨갈때 판넬과 판넬 중에 어디에 있는지 할당
        //이걸 이용하여 MoveControl 박스를 표시해줌.
        public Control MoveDragOverControl { get; set; }
